...
public class UpgradeReleaseTests {
 ...
 string project=args[0],build=Path.Combine(project,"build"),release=Path.Combine(build,"release-3.0.5");
 ...
 string output=Path.Combine(project,"reports","upgrade-3.0.5-"+DateTime.UtcNow.ToString("yyyyMMdd-HHmmss"));Directory.CreateDirectory(output);var results=new List<object>();
 ...
 results.Add(new{edition=plan.Edition,update_operations=2,old_release_detected=true,repeat_noop=true,rollback_exact=true});Console.WriteLine("UPGRADE_PASS "+plan.Edition);
 }
 Json.Save(Path.Combine(output,"validation.json"),new{Author="Neil Mitchell",Creator="Neil Mitchell",LastModifiedBy="Neil Mitchell",status="SIX_EDITION_304_TO_305_UPGRADE_REPEAT_AND_ROLLBACK_PASS",results});Console.WriteLine(output);return 0;
