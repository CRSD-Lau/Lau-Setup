...
public class UpgradeReleaseTests {
 ...
 string project=args[0],build=Path.Combine(project,"build"),release=args.Length>1?args[1]:Path.Combine(build,"release-3.0.7");
 ...
 string output=Path.Combine(project,"reports","upgrade-3.0.7-"+DateTime.UtcNow.ToString("yyyyMMdd-HHmmss"));Directory.CreateDirectory(output);var results=new List<object>();
 ...
 Json.Save(Path.Combine(output,"validation.json"),new{Author="Neil Mitchell",Creator="Neil Mitchell",LastModifiedBy="Neil Mitchell",status="SIX_EDITION_306_TO_307_UPGRADE_ONE_BYTE_REPEAT_AND_ROLLBACK_PASS",results});Console.WriteLine(output);return 0;
