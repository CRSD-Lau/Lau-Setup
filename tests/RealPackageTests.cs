...
public static class RealPackageTests {
    ...
    public static int Main(string[] args){
        ...
        try{
            ...
            foreach(var path in Directory.GetFiles(root,"*.mpq",SearchOption.AllDirectories).Where(p=>!p.Contains("frFR")))File.WriteAllBytes(path,Convert.FromBase64String("TVBRGiAAAACgAAAAAAADACAAAACgAAAACAAAAAAAAAAzMMN5KNkymLxzb5+yiE7pF6IEhMRAv8pjv3uYDOcZfi2rs0n/KaCrBG0CGFFpdt6Q+Rcj2WY9XJokR+wM0aKgsfVZrZ4Hvx59YXRsZswm0cc7Xf7ZzXv1HoxSnatryg0qR+w8s2ebsLRAZcxmcISES4BvDXjlhYGX+hYowIgcAQ=="));
            var original=Snapshot(root);var records=new List<string>();var states=new List<Dictionary<string,string>>{original};
            foreach(var settings in (online||offlineCore)?new[]{new[]{true,true,false},new[]{false,true,false},new[]{true,true,false}}:new[]{new[]{true,true,false},new[]{true,true,true},new[]{false,false,true}}){
                ...
                using(var lease=ClientLease.Acquire(root)){
                    ...
                    foreach(var id in plan.Operations.Where(o=>o.AssetId!=null&&o.SourceRelative==null).Select(o=>o.AssetId).Distinct())files[id]=d.Fetch(c.Get(id),CancellationToken.None);
