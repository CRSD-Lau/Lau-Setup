...
public class HalionTestTests {
 ...
 public static int Main(string[] a){try{
  ...
  foreach(var id in old.Assets.Keys.Where(k=>k.StartsWith("Y-"))){
   ...
   Console.WriteLine("PASS "+id);passed++;
  }
  Json.Save(Path.Combine(output,"validation.json"),new{Author="Neil Mitchell",Creator="Neil Mitchell",LastModifiedBy="Neil Mitchell",passed,apply_repeat_single_byte_rejection_restore=true});return 0;
