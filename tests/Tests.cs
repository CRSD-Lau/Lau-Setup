        ...
        try {
            ...
            Test("108 locale / edition / map install plans",()=>{
                foreach(var loc in new Fixture().Catalog.Locales)foreach(int mode in new[]{0,1,2})foreach(bool cons in new[]{false,true})foreach(bool maps in new[]{false,true}){
                    ...
                    Check(q.All(x=>x.AssetId==(maps?"MapsQ-":"LoadingQ-")+loc),"Wrong regional/core Q.");
            ...
            Test("Disabled Patch-S collisions preserve both copies",()=>{var f=new Fixture();string disabled=Path.Combine(f.Root,@"Data\patch-s.mpq.disabled");Put(disabled,"KEEP DIFFERENT DISABLED S");f.Before=f.Snapshot();Fails(()=>f.Plan(false),"different disabled");f.Original();var g=new Fixture();Directory.CreateDirectory(Path.Combine(g.Root,@"Data\patch-s.mpq.disabled"));Fails(()=>g.Plan(false),"different disabled");});
            Test("Identical disabled Patch-S is preserved through off on rollback",()=>{var f=new Fixture();foreach(var rel in new[]{@"Data\patch-s.mpq",@"Data\enUS\patch-enUS-S.MPQ"})File.Copy(Path.Combine(f.Root,rel),Path.Combine(f.Root,rel+".disabled"));f.Before=f.Snapshot();string off=f.Tx().Install(f.Plan(false),f.Files,CancellationToken.None);string on=f.Tx().Install(f.Plan(true),f.Files,CancellationToken.None);foreach(var x in f.Before.Where(x=>x.Key.EndsWith(".disabled")))Check(Hash.FileHash(Path.Combine(f.Root,x.Key))==x.Value,"On changed disabled file");f.Tx().Restore(on);f.Tx().Restore(off);f.Original();});
            Test("Disable failure after each move restores original client",()=>{int count=new Fixture().Plan(false).Operations.Count;Check(count==11,"Expected eleven operations");for(int i=0;i<count;i++){int fail=i;var f=new Fixture();var tx=f.Tx();tx.AfterMove=n=>{if(n==fail)throw new IOException("disable interruption");};Fails(()=>tx.Install(f.Plan(false),f.Files,CancellationToken.None),"disable interruption");f.Original();}});
            Test("Disabled-file rollback interruption recovers every step",()=>{int count=new Fixture().Plan(false).Operations.Count;for(int i=0;i<count;i++){int fail=i;var f=new Fixture();string j=f.Tx().Install(f.Plan(false),f.Files,CancellationToken.None);var tx=f.Tx();tx.AfterRestore=n=>{if(n==fail)throw new IOException("restore interruption");};Fails(()=>tx.Restore(j),"restore interruption");f.Tx().Restore(j);f.Original();}});
            ...
            Test("Failure after each backup move rolls back",()=>{for(int i=0;i<9;i++){int fail=i;var f=new Fixture();var t=f.Tx();t.AfterMove=n=>{if(n==fail)throw new IOException("injected interruption");};Fails(()=>t.Install(f.Plan(),f.Files,CancellationToken.None),"injected");f.Original();Check(Transaction.Pending(f.Root)==null,"Recovery incorrectly left pending.");}});
            Test("Restore survives interruption after every step",()=>{for(int i=0;i<9;i++){int fail=i;var f=new Fixture();string j=f.Tx().Install(f.Plan(),f.Files,CancellationToken.None);var t=f.Tx();t.AfterRestore=n=>{if(n==fail)throw new IOException("simulated shutdown");};Fails(()=>t.Restore(j),"simulated");Check(Transaction.Pending(f.Root)==j,"Interrupted restore not discoverable.");f.Tx().Restore(j);f.Original();}});
            ...
            Test("GUI install and restore use the real executable safely",()=>{
                ...
                using(var form=new SetupForm(f.Catalog)){
                    ...
                    Check(((CheckBox)Field(form,"maps")).Enabled&&!((CheckBox)Field(form,"maps")).Checked,"Restored map state stale.");f.Original();
                }
            });
