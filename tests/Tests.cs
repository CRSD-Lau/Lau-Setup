...
public static class Tests {
    ...
    [STAThread] public static int Main(string[] args){
        ...
        try {
            ...
            Test("Catalog-declared visual compatibility locales use English visual assets and restore exactly",()=>{
                foreach(string locale in new[]{"enGB","ptBR","itIT"}){
                    var f=new Fixture(locale);f.Catalog.ClientLocales.Add(locale);f.Catalog.VisualAssetLocales.Add(locale,"enUS");EnableMapPack(f);
                    var p=f.Plan(true,true,true);var q=p.Operations.Where(x=>x.AssetId!=null&&x.AssetId.Contains("Q-")).ToArray();
                    Check(q.Length==2&&q.All(x=>x.AssetId=="LoadingQ-enUS"),"Compatibility locale did not use the declared English visual asset.");
                    Check(q.All(x=>x.Relative.Contains("\\"+locale+"\\")||x.Relative==@"Data\patch-q.mpq"),"Compatibility locale wrote outside its active locale.");
                    Check(p.Operations.Any(x=>x.AssetId=="MapDetails-enUS"&&x.Relative==@"Data\"+locale+@"\patch-"+locale+"-T.MPQ"),"Compatibility map details used the wrong visual asset or destination.");
                    string journal=f.Tx().Install(p,f.Files,CancellationToken.None);f.Installed(p);f.Tx().Restore(journal);f.Original();
                }
            });
            ...
            Test("Configured visual compatibility locale remains the client's actual locale",()=>{foreach(string locale in new[]{"enGB","ptBR","itIT"}){var f=new Fixture();f.Catalog.ClientLocales.Add(locale);f.Catalog.VisualAssetLocales.Add(locale,"enUS");f.Catalog.Validate();File.Copy(realExe,Path.Combine(f.Root,"WoW.exe"),true);Put(Path.Combine(f.Root,@"Data\common.mpq"),"fixture");Put(Path.Combine(f.Root,@"Data\"+locale+@"\locale-"+locale+".mpq"),"fixture");Put(Path.Combine(f.Root,@"WTF\Config.wtf"),"SET locale \""+locale.ToUpperInvariant()+"\"\r\n");Check(Client.Inspect(f.Root,f.Catalog).Locale==locale,"Compatibility locale was rewritten or not normalized.");}});
