> **PRIVATE TEST BRANCH — Halion radius +65% (test v3) (Windows only)**
>
> [Download the test installer](https://github.com/CRSD-Lau/Lau-Setup/releases/tag/test-halion-radius-65-v3) · [Test instructions](HALION-TEST.md)
>
> Requires an existing original 3.0.6, test v1 or test v2 installation. Extract the entire ZIP and run `LauSetup.exe` with WoW closed. Applies only the Halion marker experiment, preserves the installed edition, and includes restore. Coldflame is unchanged. In-game boundary testing is pending. The stable release is unchanged.
