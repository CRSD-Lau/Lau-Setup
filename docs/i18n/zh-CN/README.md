...
> **带上您现有的客户端。** 这是升级，而不是完整的游戏客户端、语言包或高清模型库。运行时安装程序未捆绑。安装程序用户界面是英文的；游戏内容支持九种语言环境。
...
2。 **启动安装程序并选择您的客户端文件夹。** Windows：打开`LauSetup.exe`。 Linux：从 Wine ZIP 中提取所有四个文件并使用下面的启动器。
