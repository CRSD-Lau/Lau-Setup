...
Patch-Y contributor builds use `tools/patch_y.py` and the source contract under `patch-y/`. The tool fetches the immutable public 3.0.9 ZIP, verifies the ZIP, all six MPQs and every member, then applies only declared common or edition overlays to staged copies. Generated baselines, extracted members, candidate MPQs and reports remain ignored. See [Patch-Y development](PATCH-Y-DEVELOPMENT.md). This contributor build never writes a game client and does not alter the installer catalog.
