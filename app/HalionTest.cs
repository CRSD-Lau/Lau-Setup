...
public static class HalionTest {
 ...
 public static InstallPlan Plan(string root,Catalog old,Catalog test){
  ...
  foreach(var path in paths){string target=SafePaths.Target(root,path,client.Locale);if(!File.Exists(target))throw new IOException("Install release 3.0.6 first. Both Patch-Y placements are required.");
   ...
   if(id==null)throw new IOException("Patch-Y is modified or unsupported. This test only accepts original 3.0.6 or this exact test build.");
...
public class TestForm:Form {
 TextBox folder=new TextBox{ReadOnly=true,Dock=DockStyle.Top};Label status=new Label{AutoSize=false,Height=100,Dock=DockStyle.Top};Button choose=new Button{Text="Choose WoW folder",AutoSize=true},install=new Button{Text="Apply Halion +30% test",AutoSize=true},restore=new Button{Text="Restore previous Patch-Y",AutoSize=true};bool busy;
 public TestForm(){Text="Lau TEST ONLY - Halion radius +30%";Width=700;Height=340;MinimumSize=new Size(620,340);BackColor=Color.FromArgb(12,22,34);ForeColor=Color.White;Font=new Font("Segoe UI",10);Padding=new Padding(24);
  using(var stream=Assembly.GetExecutingAssembly().GetManifestResourceStream("LauSetup.Icon.ico"))Icon=new Icon(stream);
  var title=new Label{Text="HALION RADIUS +30% — PRIVATE TEST\nExisting edition preserved. Coldflame unchanged.\nRequires release 3.0.6. No game executable, models or maps installed.",Height=85,Dock=DockStyle.Top};
 ...
 });status.Text=undo?"Previous Patch-Y restored and verified.":result==null?"This exact Halion test is already installed.":"Halion +30% test installed and verified. Compare at the observed damage boundary. /pyversion remains 3.0.6; this window identifies the test.";
