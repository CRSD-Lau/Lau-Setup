...
public static class RangeTest {
 ...
 public static InstallPlan Plan(string root,Catalog baseline,Catalog test){
  ...
  foreach(var relative in paths){string target=SafePaths.Target(client.Root,relative,client.Locale);if(!File.Exists(target))throw new IOException("Install Lau game 3.0.8 first. Both Patch-Y placements are required.");
   ...
   if(id==null)throw new IOException("Patch-Y is modified, custom, or belongs to another test. This test only accepts the exact Lau game 3.0.8 baseline or this exact ICC range-circle test.");
 ...
 public static string Undo(string root){var test=Catalog.Embedded();var tx=new Transaction(test,null,null);string pending=Transaction.Pending(root);
  ...
  if(journal==null)throw new IOException("No ICC range-circle test backup is available. Other Lau backups are untouched.");tx.Restore(journal);return journal;
 }
...
public class RangeTestForm:Form {
 TextBox folder=new TextBox{ReadOnly=true,Dock=DockStyle.Top};Label status=new Label{AutoSize=false,Height=92,Dock=DockStyle.Top};Button choose=new Button{Text="Choose WoW folder",AutoSize=true},install=new Button{Text="Apply ICC range-circle test",AutoSize=true},restore=new Button{Text="Restore previous Patch-Y",AutoSize=true};bool busy;
 public RangeTestForm(){Text="Lau TEST ONLY - ICC 12-yard circles";Width=760;Height=370;MinimumSize=new Size(760,370);BackColor=Color.FromArgb(12,22,34);ForeColor=Color.White;Font=new Font("Segoe UI",10);Padding=new Padding(24);
  using(var stream=Assembly.GetExecutingAssembly().GetManifestResourceStream("LauSetup.Icon.ico"))Icon=new Icon(stream);
  var title=new Label{Text="ICC RANGE CIRCLES — TEST 1\nExperimental 12-yard radius / 24-yard diameter circles for BPC heroic Shadow Prison.\nVortex and Blood Nova burst markers may appear only on impact; no guaranteed advance warnings.",Height=96,Dock=DockStyle.Top};
 ...
 async Task Run(bool undo){busy=true;choose.Enabled=install.Enabled=restore.Enabled=false;status.Text="Checking and verifying files…";try{string root=folder.Text;string result=await Task.Run(()=>undo?RangeTest.Undo(root):RangeTest.Apply(root,Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"payload")));status.Text=undo?"Previous Patch-Y restored and verified.":result==null?"This exact ICC range-circle test is already installed.":"ICC range-circle test installed and verified. /pyversion stays 3.0.8; validate in-game.";}catch(Exception ex){status.Text=ex.Message;}finally{busy=false;choose.Enabled=install.Enabled=restore.Enabled=true;}}
}
}
